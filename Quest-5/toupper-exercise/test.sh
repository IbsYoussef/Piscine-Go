...
sed '/^package student/d' student/toupper.go >> temp_student.go

cat >> temp_student.go << 'EOF'

func main() {
	fmt.Println(ToUpper("Hello! How are you?"))
	fmt.Println(ToUpper("abc123"))
	fmt.Println(ToUpper("ALREADY UPPER"))
	fmt.Println(ToUpper("test"))
	fmt.Println(ToUpper(""))
	fmt.Println(ToUpper("MiXeD CaSe"))
}
EOF
...
sed '/^package solutions/d' solutions/toupper.go >> temp_solution.go
